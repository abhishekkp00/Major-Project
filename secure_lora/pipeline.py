...
from secure_lora.ingestion import ingest_directory
from secure_lora.preprocessing import preprocess_dataset
...
class SecureDatasetPipeline:
    ...
    def encrypt_dataset(
        ...
        1. Ingests raw data files from input_dir.
        2. Normalizes and preprocesses records.
        3. Writes raw preprocessed records to a secure temporary file.
        4. Encrypts the temp file to output_dir/encrypted_dataset.enc using AES-256-GCM.
        5. Computes metadata (size, SHA-256, count, schema) and writes it to dataset_metadata.json.
        6. Guarantees temporary plaintext files are securely shredded.
        7. Optionally shreds the raw source files if shred_raw is True.
        """
        ...
        try:
            ...
            secure_delete_file(temp_jsonl_path)
    ...
    def decrypt_dataset_to_file(
        ...
        WARNING: This writes the plaintext data to disk and should only be used for export/validation.
        """
        ...
        logger.info(f"Dataset successfully decrypted to: {out_path}")
    ...
    def decrypt_records(self, encrypted_file_path: Union[str, Path]) -> Generator[Dict[str, Any], None, None]:
        ...
        No plaintext file is written to disk. Ideal for feeding HuggingFace Datasets.
        """
    ...
    def decrypted_temp_file(self, encrypted_file_path: Union[str, Path]):
        ...
        Automatically shreds the file when the context block exits.
        """
