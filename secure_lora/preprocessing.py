...
def preprocess_record(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Look for common variations of instruction/prompt/question
    instruction = (
    ...
    # Look for common variations of input/context/source
    input_val = (
        record.get("input") or 
    ...
    # Filter source if it was just mapping to the source_file metadata
    if input_val == record.get("source_file"):
        input_val = ""
        
    # Look for common variations of output/response/answer/target
    output = (
    ...
    # If we have both instruction/prompt and output/response, format as instruction-tuning record
    ...
    # Check for text or content key
        ...
        # Fallback logic for arbitrary CSV/JSON columns
        # Filter out system tracking keys
        filtered_keys = [k for k in record.keys() if k not in {"source_file", "row_index", "line_number", "record_index", "block_index"}]
        
        if len(filtered_keys) == 1:
            # Single column -> treat as plain text
            standardized["text"] = clean_text(record[filtered_keys[0]])
        elif len(filtered_keys) > 1:
            # Multi-column: see if we can find any pairs or just combine
            # Let's check if there is a column containing question/answer or similar
            combined = []
...
def preprocess_dataset(raw_records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for record in raw_records:
        ...
        # Verify the record actually contains content
    ...
    logger.info(f"Preprocessed dataset: {len(raw_records)} raw records -> {len(processed_records)} clean records")
