...
|---|---|---|---|---|---|---|---|
| `ai4privacy` | AI4Privacy PII-Masking Benchmark (300k) | Hugging Face (`ai4privacy/pii-masking-300k`) | Apache-2.0 / CC-BY-4.0 | **Yes** (Span Annotations) | **No** (Fetch via HF API) | Open Web / Enterprise PII | 10,000 |
...
- **License & Attribution**: Apache-2.0 / CC-BY-4.0. Attribution to AI4Privacy project.
...
- **Primary Source**: Synthea™ Synthetic Patient Generator, MITRE Corporation.
...
## 4. Downloading & Setup Commands

To download and prepare local sample datasets or trigger Hugging Face downloads:

```bash
# Download and initialize real PII dataset samples
PYTHONPATH=. ./venv/bin/python scripts/download_real_pii.py

# Preprocess training data samples
PYTHONPATH=. ./venv/bin/python scripts/download_training_data.py
```

---

## 5. Standardized Internal JSONL Schema
...
Supported dataset sizes: `1000`, `5000`, `10000`, `25000`.
