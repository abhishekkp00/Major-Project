"""
phase3/main.py  —  CLI entry-point
------------------------------------
Orchestrates the full Phase 3 protection pipeline and exposes a command-line
interface for the three primary operations:

  protect   — encrypt, hash, sign, and package the Phase 2 adapter.
  verify    — run the authorised-device verification + controlled decryption.
  report    — print or save the security report JSON without decrypting.

Usage examples
~~~~~~~~~~~~~~
::

    # Protect (encrypt + package) the adapter produced by Phase 2:
    python -m phase3 protect

    # Verify and decrypt on the authorised device:
    python -m phase3 verify --output /tmp/restored_adapter.tar.gz

    # Print a security report:
    python -m phase3 report

Environment variables required
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  P3_DEVICE_SALT          — non-empty secret salt for key derivation.
  P3_ADAPTER_INPUT_DIR    — path to Phase 2 adapter output (default: outputs/final_adapter).
  P3_PROTECTED_OUTPUT_DIR — where to write the package (default: outputs/protected_adapter).

Optionally also set:
  P3_RSA_KEY_BITS, P3_ADAPTER_ID, P3_MODEL_REFERENCE (see config.py).
"""
...
from .adapter_encryptor import encrypt_adapter
from .device_fingerprint import get_fingerprint_hash
from .integrity import compute_file_hash, save_hash
from .key_derivation import derive_key_from_env
from .package_builder import build_package, export_package_archive
from .signature_utils import generate_dev_keypair, sign_digest, save_signature
from .verifier import VerificationError, verify_and_decrypt
...
def cmd_protect(args: argparse.Namespace) -> int:
    ...
    # Step 5 — hash
    digest = compute_file_hash(cfg.enc_path())
