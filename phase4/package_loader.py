"""
phase4/package_loader.py
-------------------------
Safely loads the protected adapter package from either a folder or a .tar.gz archive.
Extracts archives to a controlled temporary directory and enforces completeness checks.
"""
...
class IncompletePackageError(Exception):
    """Raised when the package does not contain all required files."""
    pass

class InvalidArchiveError(Exception):
    """Raised when the package archive is corrupted or invalid."""
    pass
...
class PackageLoader:
    ...
    def _extract_archive(self):
        # Prevent zip/tar bomb: check archive size before extraction
        ...
        try:
            ...
            # If the archive contained a single nested folder, adjust path
