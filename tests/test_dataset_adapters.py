...
========================
Comprehensive test suite verifying the SecureLoRA Dataset Adapter Layer:
  - BaseDatasetAdapter normalization & reproducible subsampling
  - AI4Privacy adapter (ground-truth spans, HF integration, fallback generator)
  - Synthea adapter (EHR narratives, structural metadata)
  - Synthetic adapter (legacy benchmarks & exact span generator)
  - DatasetRegistry lookup & alias resolution
  - JobOrchestrator integration with dataset adapters
  - Zero-disk-leakage & non-raw-record exposure in dashboard endpoints
"""
...
class TestDatasetAdapterLayer(unittest.TestCase):
    ...
    def test_ai4privacy_normalization_and_sampling(self):
        """Tests AI4Privacy adapter record normalization and reproducible sampling."""
        adapter = AI4PrivacyDatasetAdapter()
        recs = adapter.load_dataset(subset_size=50, seed=42, force_offline=True)
        ...
        recs_seed42_a = adapter.load_dataset(subset_size=10, seed=42, force_offline=True)
        recs_seed42_b = adapter.load_dataset(subset_size=10, seed=42, force_offline=True)
        recs_seed99 = adapter.load_dataset(subset_size=10, seed=99, force_offline=True)

        self.assertEqual([r["record_id"] for r in recs_seed42_a], [r["record_id"] for r in recs_seed42_b])

    def test_synthea_loader(self):
        """Tests Synthea clinical note adapter."""
        adapter = SyntheaDatasetAdapter()
        recs = adapter.load_dataset(subset_size=30, seed=42)
    ...
    def test_synthetic_loader(self):
        ...
        recs = adapter.load_dataset(subset_size=20, seed=42)
