...
def find_latest_checkpoint(checkpoint_dir: Path) -> Optional[str]:
    ...
    logger.info(f"Detected latest training checkpoint: {Path(latest_path).name} (Step {latest_step})")
    return latest_path
...
def rotate_checkpoints(checkpoint_dir: Path, max_to_keep: int = 2) -> None:
    ...
    for step, path in to_remove:
        try:
            logger.info(f"Rotating/removing old checkpoint directory: {path.name}")
            shutil.rmtree(path)
        except Exception as e:
            logger.error(f"Error rotating checkpoint {path.name}: {e}")
