...
logger = logging.getLogger("secure_lora.phase1.ingestion")
        ...
        if suffix == '.txt':
            ...
            with open(file_path, 'r', encoding='utf-8', errors='replace') as f:
                ...
                if isinstance(data, list):
                    ...
                    logger.warning("Unsupported JSON root type in %s: %s", file_path, type(data))
...
def ingest_directory(directory_path: Path, recursive: bool = True) -> List[Dict[str, Any]]:
    ...
    for path in directory_path.glob(glob_pattern):
        if path.is_file() and path.suffix.lower() in supported_extensions:
            logger.info("Ingesting file: %s", path.relative_to(directory_path))
            file_records = ingest_file(path)
            all_records.extend(file_records)

    logger.info("Ingested a total of %d raw records from %s", len(all_records), directory_path)
    return all_records
